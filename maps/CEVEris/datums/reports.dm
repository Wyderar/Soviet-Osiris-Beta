...
/datum/computer_file/report/recipient/crew_transfer/generate_fields()
	...
	add_field(/datum/report_field/people/from_manifest, "Имя (Старпом)")
	...
	add_field(/datum/report_field/instruction, "При неправильности заполнения следующих полей документ является недействительным")
	add_field(/datum/report_field/signature, "Подпись заявителя")
	xo_fields += add_field(/datum/report_field/signature, "Подпись Старшего Помощника")
