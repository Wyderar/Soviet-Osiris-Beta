...
/obj/machinery/computer/centrifuge/Topic(href, href_list)
	...
	switch(href_list["action"])
		if ("antibody")
			...
			var/datum/reagent/organic/blood/B = locate(/datum/reagent/organic/blood) in sample.reagents.reagent_list
			if (!B)
				state("\The [src] гудит, \"Носитель антител не обнаружен.\"", "blue")
				return 1
			...
			if (has_toxins || has_radium)
				state("\The [src] пищит, \"Скорость очистки патогена выше номинальной.\"", "blue")
				if (has_toxins)
					delay = delay/2
				if (has_radium)
					delay = delay/2
...
/obj/machinery/computer/centrifuge/proc/cure()
	...
	SSnano.update_uis(src)
	update_icon()
	ping("\The [src] pings, \"Антитела изолированы.\"")
...
/obj/machinery/computer/centrifuge/proc/print(var/mob/user)
	...
	P.info = {"
		...
		<large><meta charset=\"UTF-8\"><u>Образец:</u></large> [sample.name]<br>
"}

	if (user)
		P.info += "<u>Создано:</u> [user.name]<br>"
	...
	if (B)
		...
		var/datum/reagent/organic/antibodies/A = locate(/datum/reagent/organic/antibodies) in sample.reagents.reagent_list
		if (A)
			P.info += "Из образца крови были выделены следующие антитела: "
			P.info += antigens2string(A.data["antibodies"])
			P.info += "<br>"

	P.info += {"
	<hr>
	<u>Доп. примечание:</u> <field>
"}

	state("Рядом с компьютером распечатывается отчет о патологии.")
