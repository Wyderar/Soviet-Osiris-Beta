/obj/item/clothing/ears/headphones
	name = "headphones"
	desc = "It's time to go into another dimension."
	var/headphones_on = 0
...
/obj/item/clothing/ears/headphones/Initialize()
	. = ..()
	sound_channel = GLOB.sound_channels.request(type)
...
/obj/item/clothing/ears/headphones/OnTopic(var/user, var/list/href_list)
	...
	if(href_list["vol"])
		...
		music_volume = ((music_volume + adj) <= 0 ? 0 : ((music_volume + adj) >= 100 ? 100 : (music_volume + adj)))
		if(headphones_on)
			play_music(user)
		interact(user)
		return TOPIC_REFRESH
...
/obj/item/clothing/ears/headphones/space_pods
	name = "space pods"
	desc = "Light and compact. Don't loose it!"
	icon_state = "space_pods"
	item_state = "space_pods"