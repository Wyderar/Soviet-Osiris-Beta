...
/datum/admin_topic/centcommfaxreply/Run(list/input)
	...
	//todo: sanitize
	var/msg = input(source.owner, "Please enter a message to reply to [key_name(sender)] via secure connection. NOTE: BBCode does not work, but HTML tags do! Use <br> for line breaks.", "Outgoing message from Centcomm", "")
	if(!msg)
		return

	var/customname = input(source.owner, "Pick a title for the report", "Title")
