...
/datum/matchmaker/proc/do_matchmaking()
	...
	for(var/mob/M in to_warn)
		to_chat(M,"<span class='warning'>Вау! Вы нашли себе новые связи! Нажмите на \"<a href='byond://?src=\ref[M];show_relations=1'>Просмотреть список отношений</a>\" чтобы взглянуть на них и утвердить.</span>")
...
/datum/relation/proc/sever()
	to_chat(holder.current,"<span class='warning'>Вы больше не во взаимоотношениях с [other.holder].</span>")
	to_chat(other.holder.current,"<span class='warning'>Вы больше не во взаимоотношениях с [holder].</span>")
	other.other = null
...
/datum/relation/proc/finalize()
	finalized = 1
	to_chat(holder.current,"<span class='warning'>Вы утвердили свое взаимоотношение с [other.holder].</span>")
	to_chat(other.holder.current,"<span class='warning'>[holder] утвердил взаимоотношение с вами.</span>")
	if(other && other.finalized)
		to_chat(holder.current,"<span class='warning'>Ваше взаимоотношение с [other.holder] подтверждено!</span>")
		to_chat(other.holder.current,"<span class='warning'>Ваше взаимоотношение с [holder] подтверждено!</span>")
		...
		for(var/i=1 to 5)
			...
			if(prob(70))
				...
				M.mind.known_connections += "Видимо, [holder] и [other.holder] знают друг друга, но это не точно."
...
/mob/living/verb/see_relationship_info()
	...
	if(mind.gen_relations_info)
		dat += "<b>Что известно о вас:</b><br>[mind.gen_relations_info]<hr>"
		dat += "<b>\[F\]</b> показывает что другой персонаж утвердил взаимоотношение.<br>"
		dat += "<br>"
	for(var/datum/relation/R in relations)
		...
		dat += "<br>[R.desc]"
		dat += "<br>"
		dat += "<b>Что известно о вас:</b>[!R.finalized ?"<a href='?src=\ref[src];info_relation=\ref[R]'>Edit</a>" : ""]<br>[R.info ? "[R.info]" : " Nothing specific."]"
		if(R.other.info)
...
/mob/living/proc/see_relationship_info_with(var/mob/living/other)
	...
	for(var/datum/relation/R in relations)
		dat += "<br>[R.desc]"
		dat += "<br>"
		dat += "<b>Что известно о вас:</b><br>[R.info ? "[R.info]" : " Nothing specific."]"
		if(R.other.info)
			dat += "<br><b>Что вы знаете о них:</b><br>[R.other.info]<br>[R.other.holder.gen_relations_info]"
		dat += "<hr>"
...
/mob/living/Topic(href, href_list)
	...
	if(href_list["info_relation"])
		...
		if(istype(R))
			var/info = sanitize(input("Что о вас должны знать?","Character info",R.info) as message|null)
			if(info)
				R.info = info
				see_relationship_info()
				return 1
	if(href_list["relations_close"])
		var/ok = "Close anyway"
		ok = alert("У вас есть неутвержденные взаимоотношения. Вы можете отказаться от них или отредактировать информацию о себе только сейчас. В дальнейшем такой возможности уже не будет.","Finalize relationships","Return to edit", "Close anyway")
		if(ok == "Close anyway")
			var/list/relations = matchmaker.get_relationships(mind)
			for(var/datum/relation/R in relations)
				R.finalize()
