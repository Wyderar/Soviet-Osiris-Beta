...
/datum/computer_file/report/recipient/crew_transfer/generate_fields()
	...
	add_field(/datum/report_field/instruction, "CEV Eris - Старший Помощник")
	add_field(/datum/report_field/people/from_manifest, "Полное имя Исполнителя")
	...
	add_field(/datum/report_field/instruction, "Документ является недействительным в случае отсутствия подписи или печати.")
	add_field(/datum/report_field/signature, "Подпись Заявителя")
	xo_fields += add_field(/datum/report_field/signature, "Подпись Исполнителя")
	xo_fields += add_field(/datum/report_field/number, "Количество персонала на текущей позиции")
	xo_fields += add_field(/datum/report_field/number, "Количество персонала на запрашиваемой позиции")
...
/datum/computer_file/report/recipient/access_modification/generate_fields()
	...
	add_field(/datum/report_field/simple_text, "Продолжительной доступа")
	add_field(/datum/report_field/instruction, "Документ является недействительным в случае отсутствия подписи или печати.")
	add_field(/datum/report_field/signature, "Подпись Заявителя")
	xo_fields += add_field(/datum/report_field/signature, "Подпись Исполнителя")
	xo_fields += add_field(/datum/report_field/number, "Количество персонала на соответствующей позиции")
	xo_fields += add_field(/datum/report_field/options/yes_no, "Одобрено")
	for(var/datum/report_field/field in xo_fields)
		field.set_access(access_edit = access_hop)
...
/datum/computer_file/report/recipient/sec/investigation/generate_fields()
	..()
	add_field(/datum/report_field/instruction, "CEV Eris Ironhammer Security")
	...
	add_field(/datum/report_field/pencode_text, "Краткая сводка")
...
/datum/computer_file/report/recipient/sec/incident/generate_fields()
	..()
	add_field(/datum/report_field/instruction, "CEV Eris Ironhammer Security")
	add_field(/datum/report_field/instruction, "Заполняется дежурным сотрудником, отвечающим за инцидент. Отчет должен быть подписан и представлен до окончания смены!")
	add_field(/datum/report_field/people/from_manifest, "Сотрудник, составивший отчет")
	add_field(/datum/report_field/simple_text, "Тип происшествия/преступления")
	add_field(/datum/report_field/date, "Дата инцидента")
	add_field(/datum/report_field/time, "Время инцидента")
	add_field(/datum/report_field/people/list_from_manifest, "Ассистирующий сотрудник")
	add_field(/datum/report_field/simple_text, "Местоположение")
	add_field(/datum/report_field/pencode_text, "Персонал, вовлеченный в инцидент", "\[small\]\[i\](Ж-Жертва, П-Подозреваемый, С-Свидетель, Пр-Пропавший, А-Арестованный, М-Мертвец)\[/i\]\[/small\]")
	add_field(/datum/report_field/pencode_text, "Описание предметом/Материального ущерба", "\[small\]\[i\](П-Поврежденный, У-Улика, Пр-Потерянный, В-Восстановленный, Ук-Украденный)\[/i\]\[/small\]")
	add_field(/datum/report_field/pencode_text, "Полное описание инцидента")
	add_field(/datum/report_field/signature, "Подпись сотрудника, составившего отчет")
	set_access(access_edit = access_security)
...
/datum/computer_file/report/recipient/sec/evidence/generate_fields()
	...
	add_field(/datum/report_field/pencode_text, "Список конфискованных предметов")
	set_access(access_edit = access_security)
	temp_field = add_field(/datum/report_field/signature, "Подпись Сержанта")
	temp_field.set_access(access_edit = list(access_security, access_armory))
	temp_field = add_field(/datum/report_field/signature, "Подпись Инспектора")
...
/datum/computer_file/report/recipient/docked/generate_fields()
	..()
	add_field(/datum/report_field/instruction, "CEV Eris Asters Guild")
	add_field(/datum/report_field/instruction, "Общая информация")
	add_field(/datum/report_field/date, "Дата")
	add_field(/datum/report_field/simple_text, "Название судна")
	add_field(/datum/report_field/simple_text, "Владелец/Пилот судна")
	add_field(/datum/report_field/simple_text, "Назначение судна")
	add_field(/datum/report_field/people/from_manifest, "Состыковку произвел")
	add_field(/datum/report_field/instruction, "Общая информация груза")
	add_field(/datum/report_field/pencode_text, "Перечислите вид груза, находящегося на судне")
	add_field(/datum/report_field/instruction, "Информация об опасном грузе судна")
	add_field(/datum/report_field/options/yes_no, "Вооружение?")
	add_field(/datum/report_field/options/yes_no, "Живой груз?")
	add_field(/datum/report_field/options/yes_no, "Биологический материал?")
	add_field(/datum/report_field/options/yes_no, "Химическая или радиационная опасность?")
	add_field(/datum/report_field/signature, "Для разрешения на посещение судна, распишитесь/поставьте печать здесь")
	add_field(/datum/report_field/instruction, "Отстыковка")
...
/datum/computer_file/report/recipient/nt/proc/add_header()
	add_field(/datum/report_field/simple_text, "Судно", "CEV Eris")
	add_field(/datum/report_field/date, "Дата заполнения")
	add_field(/datum/report_field/time, "Время заполнения")
...
/datum/computer_file/report/recipient/nt/incident/generate_fields()
	..()
	add_header()
	add_field(/datum/report_field/pencode_text, "Краткая сводка инцидента")
	add_field(/datum/report_field/pencode_text, "Детали инцидента")
...
/datum/computer_file/report/recipient/nt/incident/ship
	form_name = "E-3203"
	title = "Отчет об инциденте на корабле"
	available_on_ntnet = 1

/datum/computer_file/report/recipient/nt/incident/ship/generate_fields()
	..()
	add_field(/datum/report_field/pencode_text, "Вовлеченные отделы")
	add_signatures()


/datum/computer_file/report/recipient/nt/incident/personnel
	form_name = "E-3205"
	title = "Отчет об инциденте с персоналом"
	available_on_ntnet = 1
...
//	form_name = "C-3201"
//	title = "Отчет о корпоративном инциденте"
//	available_on_ntnet = 1

///datum/computer_file/report/recipient/nt/incident/asset/generate_fields()
//	..()
//	add_field(/datum/report_field/pencode_text, "Травмы сотрудников")
//	add_field(/datum/report_field/pencode_text, "Потери среди оборудования")
//	add_signatures()
...
/datum/computer_file/report/recipient/nt/volunteer
	form_name = "E-1443"
	title = "Волонтерство для исследований"
	available_on_ntnet = 1
...
/datum/computer_file/report/recipient/nt/volunteer/generate_fields()
	...
	add_header()
	add_field(/datum/report_field/people/from_manifest, "Полное имя")
	add_field(/datum/report_field/simple_text, "Описание процедуры")
	add_field(/datum/report_field/simple_text, "Выплачиваемая компенсация")
	add_field(/datum/report_field/people/list_from_manifest, "Исследователи")
	add_field(/datum/report_field/instruction, "Подписывая документ, \"Волонтер\" соглашается с освобождением Исследовательской корпорации от любой ответственности за причиненные травмы, ущерб, утрату имущества или различные побочные эффекты.")
	add_field(/datum/report_field/signature, "Подпись волонтера:")
	temp_fields += add_field(/datum/report_field/signature, "Подпись представителя корпорации")
