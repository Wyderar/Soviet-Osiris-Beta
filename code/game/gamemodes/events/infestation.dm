...
/datum/event/infestation/announce()
	...
	if (prob(8))
		command_announcement.Announce("На борту корабля [station_name()] обнаружены неизвестные формы жизни. Персоналу требуется обеспечить безопасность любого внешнего шлюза, включая воздуховоды и вентиляцию.", "Предупреждение: Неизвестные формы жизни", new_sound = 'sound/AI/aliens.ogg')
	else
		...
		if (areanames.len)
			switch(severity)
				if (EVENT_LEVEL_MUNDANE)
					command_announcement.Announce("Биосканы указывают, что [chosen_mob] [chosen_verb] [english_list(areanames)]. Уничтожьте их, прежде чем они начнут влиять на производительность корабля.", event_name, new_sound = 'sound/AI/vermin.ogg')
				if (EVENT_LEVEL_MODERATE)
					command_announcement.Announce("Биосканы указывают, что [chosen_mob] [chosen_verb] [english_list(areanames)]. Сотрудникам Ironhammer рекомендуется подходить с осторожностью.", event_name, new_sound = 'sound/AI/vermin.ogg')
				if (EVENT_LEVEL_MAJOR)
					command_announcement.Announce("ВНИМАНИЕ ВСЕМУ ЭКИПАЖУ: Биосканы указывают, что [chosen_mob] [chosen_verb] [english_list(areanames)]. Экипажу рекомендуется немедленно покинуть данные районы.", event_name, new_sound = 'sound/AI/vermin.ogg')
