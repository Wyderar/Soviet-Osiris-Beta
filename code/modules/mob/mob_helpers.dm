...
proc/slur(phrase)
	...
	while(counter>=1)
		...
		if(rand(1,3)==3)
			if(newletter == "o")	newletter="u"
			if(newletter == "s")	newletter="ch"
			if(newletter == "a")	newletter="ah"
			if(newletter == "c")	newletter="k"
		switch(rand(1,15))
			if(1,3,5,8)	newletter="[newletter]"
			if(2,4,6,15)	newletter="[newletter]"
			if(7)	newletter+="'"
...
/proc/stutter(n)
	...
	while(p <= n)//while P, which starts at 1 is less or equal to N which is the length.
		...
		if (prob(80) && (n_letter in LIST_OF_CONSONANT))
			if (prob(10))
